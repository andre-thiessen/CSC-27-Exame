...
contract PasskeyWalletFactory {
    ...
    constructor(IEntryPoint _entryPoint) {
        accountImplementation = new PasskeyWallet(_entryPoint);
    }
    ...
    function createAccount(
        ...
        uint256 pubKeyX,
        uint256 pubKeyY
    ) public returns (PasskeyWallet ret) {
        address addr = getAddress(salt, keyId, pubKeyX, pubKeyY);
        uint codeSize = addr.code.length;
        if (codeSize > 0) {
            return PasskeyWallet(payable(addr));
        }
        return
            ret = PasskeyWallet(
                payable(
                    new ERC1967Proxy{salt: bytes32(salt)}(
                        ...
                        abi.encodeCall(
                            PasskeyWallet.initialize,
                            (keyId, pubKeyX, pubKeyY)
                        )
                    )
                )
            );
